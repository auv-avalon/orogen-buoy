...
#    wall_servoing.start
    ...
    wall_follower.search_direction = 0.25 * Math::PI
...
###########################CONNECTIONS
  ...
  buoy_detector = Orocos::TaskContext.get "buoy_detector"
  controller = Orocos::TaskContext.get "buoy_wall_servoing"

  front_cam.frame.connect_to buoy_detector.frame

  buoy_detector.hValueMin = 171
  buoy_detector.hValueMax = 56
  buoy_detector.sValueMin = 155
  buoy_detector.sValueMax = 255
  buoy_detector.vValueMin = 155
  buoy_detector.vValueMax = 255
  buoy_detector.houghMinCircle = 7
  buoy_detector.hSmooth = 11
  buoy_detector.sSmooth = 11
  buoy_detector.vSmooth = 11




  buoy_detector.configure
  buoy_detector.start

  imu.pose_samples.connect_to controller.orientation_samples
  buoy_detector.buoy.connect_to controller.buoy_samples
  servoing_wall_detector.wall.connect_to controller.wall_samples
  ...
  on_search = true
    loop do
        if(on_search and (controller.state == :BUOY_SERVOING))
            print "Boje gefunden"
            on_search = false
            
            wall_follower.aligned_velocity_command.disconnect_from(avc.cmd_in)
            wall_follower.world_command.disconnect_from(world_to_aligned.cmd_in)
            ...
            controller.world_cmd.connect_to world_to_aligned.cmd_in
            controller.aligned_position_cmd.connect_to apc.cmd_in
        end
        sleep 0.2
    end
